...
# creating data frame of names and ID's to be used later for results
player_names <- data %>%
  select(PLAYER_NAME, BATTER_ID)
...
# taking the dataframe of player names earlier and joining it with the pitch mix proportions dataframe
Pitch_mix <- Pitch_mix %>%
 left_join(player_names, by = "BATTER_ID") 
...
Pitch_mix <- Pitch_mix %>%
  ...
  ungroup() %>%
  mutate(GAME_YEAR = 2024) # creating a column GAME_YEAR to represent the year these proportions are in effect for
...
write.csv(Pitch_mix, "pitch_mix_2024.csv", row.names = FALSE)

# Showing the first 10 results
knitr::kable(head(Pitch_mix, 10))
```
